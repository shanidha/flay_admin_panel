                ...
                ElevatedButton(
                  onPressed: () {
                    context.read<ShellBloc>().add(
                        const ShellSectionChanged(ShellSection.addProduct));
                    // Get.find<DashboardController>().changeSection(4);
                  },
...
DataRow _buildRow(BuildContext context, ProductEntity p) {
  final sizeInfo = p.sizes.isEmpty ? 'No size' : 'Size: ${p.sizes.join(', ')}';
  ...
  return DataRow(
    cells: [
      ...
      DataCell(Text('Rs ${p.price.toStringAsFixed(0)}')),
      DataCell(
        Column(
          mainAxisAlignment: MainAxisAlignment.center,
          crossAxisAlignment: CrossAxisAlignment.start,
      ...
      DataCell(
        Row(
          children: [
            ...
            IconButton(
              icon: const Icon(Icons.delete_outline, color: Color(0xFFFF6C30)),
              onPressed: () {
                context.read<ProductBloc>().add(
                      DeleteProductSubmitted(p.id),
                    );
              },
            ),
            const SizedBox(width: 8),
            IconButton(
              icon: const Icon(Icons.edit, color: Color(0xFFFF6C30)),
              onPressed: () {
                Navigator.of(context).push(
                  MaterialPageRoute(
                    builder: (_) => AddProductContentScreen(product: p),
                  ),
                );
              },
            ),
